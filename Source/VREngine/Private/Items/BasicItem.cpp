...
void ABasicItem::OnComponentHit(UPrimitiveComponent* HitComponent, AActor* OtherActor, UPrimitiveComponent* OtherComp, FVector NormalImpulse, const FHitResult& Hit)
{
	...
	if (IsValid(OtherActor))
	{
		...
		{
			float damage = FMath::Abs(Velocity) * DamageMultiplier;
...
void ABasicItem::BeginPlay()
{
	...
	TSubclassOf<ABaseMotionController>M;
...
bool ABasicItem::CheckHandDistance(float MaxDistance)
{
	if (IsValid(MotionControllerBP->GrabSphereComponent))
	{
		FVector Vector = FVector(MainMesh->GetComponentLocation() - MotionControllerBP->GrabSphereComponent->GetComponentLocation());
...
void ABasicItem::GrabAttachTo()
{
	...
	if (bIsGrabbed)
	{
		FTransform Temp = GetWorldPickupTransform();
...
void ABasicItem::GrabPhysicsHandle()
{
	...
	float PhysicsMass = MainMesh->GetMass() * 10.0f;
...
FTransform ABasicItem::GetRelativePickupTransform()
{
...
void ABasicItem::Tick(float DeltaTime)
{
	...
	FVector LocalCurrentPosition = MainMesh->GetComponentLocation();
